if ($('#monkey_404').length > 0 ) {
  var monkeySVG = Snap('#monkey_404');

  var tail = monkeySVG.select('#tail');

  var numberZero = monkeySVG.select('#number_0-shadow');
  ...
  //var bigC = monkeySVG.circle(400,200,40).attr({ stroke: 'red', 'strokeWidth': 0, fill: 'red'});


  var bigC = monkeySVG.select('#sword-shadow');

  numberZero.attr({ mask: bigC });
}
